...
class UnpackenPacket:
    def __init__(self,field,generation_size,num_packets):
        ...
        self.rank_buffer_zp = {i:0 for i in range(num_packets// (generation_size//2) )}
    ...
    def unpacken(self,packet,checkString="BBased"):
        ...
        print "self.generation_size",self.generation_size
        print "ID",ID,"blockNum = ID // (self.generation_size//2)",ID // (self.generation_size//2) 
        if checkString == "SW":
        ...
        if Type == 0x01: 
            ...
            # print "self.generation_size1",self.generation_size
            temp_GS = packet[2]
            # print "self.generation_size2",self.generation_size
            ...
            if checkFull(ID):
                ...
                coeff_list = [item for item in packet[3:3+temp_GS]]
                sizes_list = packet[3+temp_GS : 3+2*temp_GS]
                coded_data = packet[3+2*temp_GS]
    ...
    def decoderProcess(self,ID, coeff_list_m, coded_data_m, sizes_list):
        ...
        # # print "self.data_buffer[index]",self.data_buffer
        # print "coeff_list[i]",coeff_list
