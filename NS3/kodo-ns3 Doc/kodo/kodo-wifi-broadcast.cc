...
int main (int argc, char *argv[])
{
  ...
  double rss = -93;  // -dBm
  uint32_t packetSize = 18000; // bytes 1000
  double interval = 1.0; // seconds
  uint32_t generationSize = 16;
  uint32_t users = 4; // Number of users
...
//    users, generationSize, packetSize, source, sinks);
  ...
  Broadcast wifiBroadcast (kodocpp::codec::sliding_window, fieldMap[field],
    users, generationSize, packetSize, source, sinks);
...
//   kodo-wifi
//  AnimationInterface anim("kodo-wifi-text.xml");
